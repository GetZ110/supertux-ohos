...
| ✅ | 语言可切换（Options → Locale → Select Language）：字体改为内存缓冲加载后，切到中文/日文等 CJK 语言不再卡死 |
| ✅ | 长菜单可用手指拖动滚动，点按才选中（触摸不再当悬停用）；进入语言列表时高亮落在当前语言上 |
...
scripts/build-supertux-hap.sh --key-alias sdl3demo   # 5. 打包 + 签名 + 安装 + 启动 + hilog + 截图
```

只出 HAP 不上机：`scripts/build-supertux-hap.sh --skip-install`；
想带上音乐：`--include-music`（data.zip 从 ~139 MB 涨到 ~287 MB）。

> `--key-alias` 必须和密钥库里的别名一致（本仓库示例密钥库 `signing/sdl3demo.p12` 的别名是
> `sdl3demo`；脚本默认 `debugKey`）。别名不对时签名会失败，脚本现在会**直接报错退出** ——
> 以前它只是打一行日志，然后继续安装上一次留下的旧 HAP，看起来像构建成功，很容易把人带沟里。
...
6. **字体走内存流**：`data.zip` 里的字体是 deflate 压缩的，而 PhysFS **无法在压缩条目里 seek** ——
   任何向后 seek 都会从头重新解压（`physfs_archiver_zip.c` 的 `ZIP_seek()`）。FreeType 打开字体时
   要来回 seek 几十次，于是 `NotoSansCJKjp-Medium.otf`（16.5 MB）每打开一次要 **3.3 秒**；
   语言菜单会为 cmn/ja/zh_CN/zh_TW 各建一个 `TTFFont`，四次就是 ~14 秒，主线程被鸿蒙看门狗
   （THREAD_BLOCK_6S）直接 SIGKILL。现在字体先整份读进内存再交给 SDL_ttf，并按文件名缓存，
   四个语言共用一份：实测同一次菜单构造从 ~14 s 降到 **17–19 ms**。
7. **触摸不当作悬停**：菜单原本靠 hover 命中来移动高亮，手指一碰就选中手指下面那一项，列表长了
   既没法看全也没法滚动。现在 `ScreenManager` 把合成的鼠标事件标成 `SDL_TOUCH_MOUSEID`，
   菜单据此区分触摸：触摸只滚动（`Menu::scroll_by()` 的独立偏移，**不动选中项**）、**抬起且没拖动
   才算点按**（点按才把高亮挪到手指那一项并触发），鼠标/键盘的 hover 行为不变。
